...
logger = logging.getLogger('letter_parser_no_ocr')
...
class LetterParser:
    ...
    def __init__(self, filepath, filename):
        ...
        if not self.ocr_flag:
            ...
            #self.contents = None
            self.header_list, self.footer_list = self.get_header_footer()
            self.name = self.get_metadata_wrapper(self.get_author)
            ...
            if not self.university:
                self.university = self.get_metadata_wrapper(self.get_affiliation)
        else:
            self.file_postprocess()
            raise ValueError('Need OCR engine.')
        ...
        # self.race = choice(['Asian', 'American', 'Muslim'])
    ...
    def file_preprocess(self):
        ...
        tree = etree.parse(file_xml, parser=parser)
    ...
    def get_header_footer(self):
        body_top = self.texts[self.refined_id[0]].get('top')
        header_list = []
        footer_list = []
        for pos, val in enumerate(self.texts):
            if pos not in self.refined_id:
                if pos < self.refined_id[0]:
                    if val.text.strip():
                        #header_list.append(pos)
                        header_list.append(val.text.strip())
                elif pos > self.refined_id[-1]:
                    if val.text.strip():
                        #footer_list.append(pos)
                        footer_list.append(val.text.strip())
                elif self.texts[pos].get('top') <= body_top:
                    if val.text.strip():
                        #header_list.append(pos)
                        header_list.append(val.text.strip())
                else:
                    if val.text.strip():
                        #footer_list.append(pos)
                        footer_list.append(val.text.strip())
        #header_list = [val.text.strip() for pos, val in enumerate(self.texts) if pos not in self.refined_id
        #               and self.texts[pos].get('top') <= body_top and val.text.strip()]
        #footer_list = [val.text.strip() for pos, val in enumerate(self.texts) if self.texts[pos].get('top') > body_top and val.text.strip()]
        self.refine_header = True
        if not header_list:
            logger.debug('File: %s, No header' % self.filename)
            #raise ValueError('No header, please check refine methods for file %s\n' % self.filename)
        if not footer_list:
            logger.debug('File: %s, No footer' % self.filename)
            #raise ValueError('No footer, please check refine methods for file %s\n' % self.filename)
        return header_list, footer_list

    def get_metadata_wrapper(self, func):
        footer_level = func(self.footer_list)
        if footer_level:
            return footer_level
        if self.refine_header:
            self.header_refine()
        header_level = func(self.header_list)
        if header_level:
            return header_level
    ...
    def get_department(header_list):
        ...
        other_strings = ['department[s]?', 'dept', 'center', 'school']
        for string in other_strings:
            string_text = [val for val in header_list
                           if val if re.findall(r'%s' % string, val, re.IGNORECASE)]
            if string_text:
                departments = [re.findall(r'[a-zA-Z& ]*%s[a-zA-Z& ]*' % string, val, re.IGNORECASE)[0].strip() for val in string_text]
        ...
        if professor:
            departments = [re.sub(r'.*((?:Professor|Prof)\s{,2}of)', r'Department of', val).strip() for val in professor]
            #departments = [re.sub(r'(?:Adjunct)?\s{,2}(?:Professor|Prof)\s{,2}of([a-zA-Z& ]+)', r'Department of\1',
            #              val, flags=re.I).strip() for val in professor]
            if len(departments) > 1:
                departments = '|'.join(departments)
            else:
                departments = ''.join(departments)
            return departments
            #return re.sub(r'(?:Adjunct)?\s{,2}(?:Professor|Prof)\s{,2}of([a-zA-Z& ]+)', r'Department of\1',
    ...
    def get_university(header_list):
        ...
        other_strings = ['universi', 'univ', 'institut']
        for string in other_strings:
            string_text = [val for val in header_list
                           if val if re.findall(r'%s' % string, val, re.IGNORECASE)]
            if string_text:
    ...
    def get_author(self, header_list):
        for line in header_list:
            name = self.get_entity('PERSON', line)
            if name:
                return name
        return None

    def get_affiliation(self, header_list):
        name_list = []
        for line in header_list:
            line = re.sub(r'\s{2,}', ' ', re.sub(r'[,]|[.]|[:]', ' ', line))
            name = self.get_entity('ORGANIZATION', line)
            if name:
    ...
    @staticmethod
    def get_entity(entity, line):
        for sent in nltk.sent_tokenize(line):
            tokens = nltk.tokenize.word_tokenize(sent)
            tags = st.tag(tokens)
            name = " ".join([tag[0] for tag in tags if tag[1] == entity])
            return str(name)
    ...
    def height_refine(self, texts):
        ...
        height_refined_list = [pos for pos, text in enumerate(texts)
                               if int(text.get('height')) < (height_target+3) and int(text.get('height')) > (height_target-3)]
    ...
    def header_refine(self):
        name_pos = -1
        for pos, val in enumerate(self.header_list):
            if re.findall(r'P[H|h][.]?[D|d][.]?', val):
                name_pos = pos
                break
        if name_pos == -1:
            for pos, val in enumerate(self.header_list):
                temp = self.get_entity('PERSON', val)
                if temp:
                    name_pos = pos
                    break
        if name_pos == -1:
            name_pos = 0
        self.header_list = [val for pos, val in enumerate(self.header_list[name_pos + 1:]) if pos < 5]
        self.refine_header = False
...
# TODO: special character encoding
